...
#
# Environment:
#   NAAN_SRCDIR       project root (default: packaging/../..)
#   NAAN_FORCE_LOCAL  skip can-local check (set on a build host by gh-makerelease)
set -euo pipefail

_LIBDIR=$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)
NAAN_SRCDIR=${NAAN_SRCDIR:-$(cd "$_LIBDIR/../.." && pwd)}
...
find_build_host_file() {
    ...
    for k in $(_kind_fallbacks "$kind"); do
        for base in "$NAAN_SRCDIR/.config/naan" "$HOME/.config/naan"; do
...
cmd_dump() {
    local file=${1:-}
    if [[ -z $file ]]; then
        file=$(find_build_host_file "${NAAN_PACKAGING:-win32}") || {
    ...
    if ((BH_N > 0)); then
        ...
        echo "last_shell=$(last_shell "${NAAN_PACKAGING:-}")"
...
cmd_run() {
    ...
    local k
    for k in $(_kind_fallbacks "$kind"); do
        echo "    $NAAN_SRCDIR/.config/naan/$k.build-host" >&2
        echo "    $HOME/.config/naan/$k.build-host" >&2
    done
    return 2
}
